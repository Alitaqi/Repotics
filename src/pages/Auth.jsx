...
export default function Auth() {
  ...
  // Redirect automatically if user is logged in
  useEffect(() => {
    if (currentUser?._id) {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      let userData;
      if (isLogin) {
        userData = await loginUser({ credential: form.email, password: form.password }).unwrap();
      } else {
        userData = await registerUser(form).unwrap();
      }

      // Save user in Redux — navigation will happen in useEffect
        ...
        >
            ...
            {!isLogin && (
              <>
                <div>
                  ...
                  <Input id="name" name="name" placeholder="Ali Taqi" value={form.name} onChange={handleChange} />
                </div>

                <div>
                  <Label htmlFor="username" className="mb-2 text-gray-700">Username</Label>
                  <Input id="username" name="username" placeholder="alitaqi349" value={form.username} onChange={handleChange} />
                ...
                <div>
                  ...
                  <Input id="dob" name="dob" placeholder="YYYY-MM-DD" value={form.dob} onChange={handleChange} />
                </div>
              </>
            )}

            <div>
              <Label htmlFor="email" className="mb-2 text-gray-700">{isLogin ? "Email / Username" : "Email"}</Label>
              <Input id="email" name="email" placeholder="example@mail.com" value={form.email} onChange={handleChange} />
            </div>

            <div>
              <Label htmlFor="password" className="mb-2 text-gray-700">Password</Label>
              <Input type="password" id="password" name="password" placeholder="********" value={form.password} onChange={handleChange} />
            </div>
          ...
          <p className="mt-4 text-sm text-center text-gray-600">
            ...
            <button
              onClick={() => {
                setIsLogin(!isLogin);
                setForm({ name: "", username: "", email: "", password: "", dob: "" });
